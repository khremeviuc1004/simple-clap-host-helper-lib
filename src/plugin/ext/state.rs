...
impl State {
    ...
    pub fn save(&self, plugin: &Plugin) -> Result<Vec<u8>> {
        let stream = OutputStream::new();

        if unsafe_clap_call! { self.state.as_ptr()=>save(plugin.as_ptr(), &stream.vtable) } {
            Ok(stream.into_vec())
        } else {
    ...
    pub fn save_buffered(&self, plugin: &Plugin, max_bytes: usize) -> Result<Vec<u8>> {
    ...
    pub fn load(&self, plugin: &Plugin, state: &[u8]) -> Result<()> {
        let stream = InputStream::new(state);

        if unsafe_clap_call! { self.state.as_ptr()=>load(plugin.as_ptr(), stream.vtable()) } {
            Ok(())
        } else {
    ...
    /// time. Returns an error if the plugin returned `false`.
    pub fn load_buffered(&self, plugin: &Plugin, state: &[u8], max_bytes: usize) -> Result<()> {
