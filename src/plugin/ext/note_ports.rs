...
impl Extension<&Plugin> for NotePorts {
...
impl NotePorts {
    ...
    /// are consistent.
    pub fn config(&self, plugin: &Plugin) -> Result<NotePortConfig> {
        let mut config = NotePortConfig::default();

        let note_ports = unsafe { self.note_ports.as_ref() };
        let num_inputs = unsafe_clap_call! { note_ports=>count(plugin.as_ptr(), true) };
        let num_outputs = unsafe_clap_call! { note_ports=>count(plugin.as_ptr(), false) };
        ...
        for i in 0..num_inputs {
            ...
            let success =
                unsafe_clap_call! { note_ports=>get(plugin.as_ptr(), i, true, &mut info) };
            if !success {
                anyhow::bail!(
