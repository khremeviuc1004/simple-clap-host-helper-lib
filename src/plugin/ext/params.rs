...
impl Extension<&Plugin> for Params {
...
impl Params {
    ...
    fn status(&self, plugin: &Plugin) -> PluginStatus {
        plugin.status()
    }

    /// Get a parameter's value.
    pub fn get(&self, plugin: &Plugin, param_id: clap_id) -> Result<f64> {
        let mut value = 0.0f64;
        if unsafe_clap_call! { self.params.as_ptr()=>get_value(plugin.as_ptr(), param_id, &mut value) }
        {
            Ok(value)
        } else {
            anyhow::bail!(
    ...
    /// invalid UTF-8.
    pub fn value_to_text(&self, plugin: &Plugin, param_id: clap_id, value: f64) -> Result<Option<String>> {
    ...
    pub fn text_to_value(&self, plugin: &Plugin, param_id: clap_id, text: &str) -> Result<Option<f64>> {
    ...
    pub fn info(&self, plugin: &Plugin) -> Result<ParamInfo> {
        let mut result = BTreeMap::new();

        let params = unsafe { self.params.as_ref() };
        let num_params = unsafe_clap_call! { params=>count(plugin.as_ptr()) };
        ...
        for i in 0..num_params {
            ...
            let success =
                unsafe_clap_call! { params=>get_info(plugin.as_ptr(), i, &mut info) };
            if !success {
                anyhow::bail!(
