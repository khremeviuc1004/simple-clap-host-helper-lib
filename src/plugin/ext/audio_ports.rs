...
impl Extension<&Plugin> for AudioPorts {
...
impl AudioPorts {
    ...
    pub fn config(&self, plugin: &Plugin) -> Result<AudioPortConfig> {
        ...
        let num_inputs = unsafe_clap_call! { audio_ports=>count(plugin.as_ptr(), true) };
        let num_outputs = unsafe_clap_call! { audio_ports=>count(plugin.as_ptr(), false) };
        ...
        for i in 0..num_inputs {
            ...
            let success =
                unsafe_clap_call! { audio_ports=>get(plugin.as_ptr(), i, true, &mut info) };
            if !success {
                anyhow::bail!(
        ...
        for i in 0..num_outputs {
            let mut info: clap_audio_port_info = unsafe { std::mem::zeroed() };
            let success =
                unsafe_clap_call! { audio_ports=>get(plugin.as_ptr(), i, false, &mut info) };
            if !success {
                anyhow::bail!(
...
impl AudioPortConfig {
    ...
    pub fn create_buffers(&self, buffer_size: usize) -> (Box<Vec<Vec<Vec<f32>>>>, Box<Vec<Vec<Vec<f32>>>>) {
        ...
        (Box::new(input_buffers), Box::new(output_buffers))
    }
}
